...
int AlsaCommonStream::open_pcm(snd_pcm_stream_t direction)
{
  ...
  if (!stream_info_.device.empty()) {
    preferred = stream_info_.device;
  } else {
    preferred = (direction == SND_PCM_STREAM_PLAYBACK) ? ALSA_DEFAULT_PLAYBACK_DEVICE
                                                        : ALSA_DEFAULT_CAPTURE_DEVICE;
  }

  int err = snd_pcm_open(&pcm_handle_, preferred.c_str(), direction, 0);
  if (err >= 0) {
    resolved_device_ = preferred;
    LOGD("opened device '%s'", resolved_device_.c_str());
    return 0;
  }

  // Fall back to "default"
  LOGI("device '%s' unavailable (%s), falling back to 'default'",
       preferred.c_str(), snd_strerror(err));
  err = snd_pcm_open(&pcm_handle_, "default", direction, 0);
  if (err < 0) {
    LOGE("snd_pcm_open(default): %s", snd_strerror(err));
    return err;
  }
  resolved_device_ = "default";
  LOGD("opened fallback device 'default'");
  return 0;
}
...
AlsaPlaybackStream::AlsaPlaybackStream(const AudioStreamInfo & info,
    stream_event_callback_func cb)
...
{
  const size_t frame_bytes = stream_info_.channels * (stream_info_.format / 8);
  const size_t period_bytes = ALSA_DEFAULT_PERIOD_FRAMES * frame_bytes;
  const std::vector<uint8_t> silence(period_bytes, 0);
...
AlsaCaptureStream::AlsaCaptureStream(const AudioStreamInfo & info,
    stream_event_callback_func cb)
  : AlsaCommonStream(info, cb)
{
